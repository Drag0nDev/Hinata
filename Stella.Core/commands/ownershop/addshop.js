...
module.exports = {
    ...
    examples: ['s!as -n diamond ring -p 500 -c badge -im'],
    run: async (bot, message, args) => {
        ...
        if (db.category === null) {
            embed.setDescription(`The category **${item.category}** doesn't exists!`)
        ...
        if (db.shop !== null) {
            ...
            if (db.shop.image !== null)
                embed.addField('Existing item',
                    ...
                    `**Image:**`)
                    .attachFiles([`./Stella.Core/misc/images/inventory/${db.shop.image}`])
